...
class RequestMiddleware(ABC):
    ...
    @abstractmethod
    def transform(self, requests: Iterable[Request], response: Response) -> Iterable[Request]: pass
...
class FilterMiddleware(RequestMiddleware):
    ...
    def transform(self, requests: Iterable[Request], response: Response) -> Iterable[Request]:
...
class GenerationMiddleware(RequestMiddleware):
    ...
    def transform(self, requests: Iterable[Request], response) -> Iterable[Request]:
        if response:
...
class ChainMiddleware(RequestMiddleware):
    ...
    def __init__(self, *middlewares: RequestMiddleware):
        self._middlewares: Tuple[RequestMiddleware] = middlewares
